...
Prism.languages.vcconditions = Prism.languages.extend('clike', {
	'keyword': /\b(aap)\b/,
...
Prism.languages.insertBefore('vcconditions', 'keyword', {
	'regex': {
		pattern: /(^|[^/])\/(?!\/)(\[.+?]|\\.|[^/\\\r\n])+\/[gimyu]{0,5}(?=\s*($|[\r\n,.;})]))/,
		lookbehind: true
	}
...
Prism.languages.insertBefore('vcconditions', 'class-name', {
	'template-string': {
		pattern: /`(?:\\`|\\?[^`])*`/,
		inside: {
			'interpolation': {
				...
				inside: {
					...
					rest: Prism.languages.vcconditions
				}
			},
			'string': /[\s\S]+/
		}
...
if (Prism.languages.markup) {
	Prism.languages.insertBefore('markup', 'tag', {
		'script': {
			...
			lookbehind: true,
			inside: Prism.languages.vcconditions,
			alias: 'language-vcconditions'
		}
	});
}

Prism.languages.js = Prism.languages.vcconditions;
